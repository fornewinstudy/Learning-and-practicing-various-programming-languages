...
ListView gfnListView=0;

pnNtQuerySystemInformation      NtQuerySystemInformation    =0;
//pnGetSiteSidFromToken           GetSiteSidFromToken         =0;
pnNtClose                       NtClose                     =0;
pnNtOpenThread                  NtOpenThread                =0;
pnRtlLargeIntegerToChar         RtlLargeIntegerToChar       =0;
pnRtlAnsiStringToUnicodeString  RtlAnsiStringToUnicodeString=0;
pnRtlUnicodeStringToInteger     RtlUnicodeStringToInteger   =0;
pnRtlTimeToElapsedTimeFields    RtlTimeToElapsedTimeFields  =0;
pnRtlInitUnicodeString          RtlInitUnicodeString        =0;
pnNtQueryInformationToken       NtQueryInformationToken     =0;
pnEndTask                       EndTask                     =0;
pnRunFileDlg                    RunFileDlg                  =0;
pnIsHungAppWindow               fpnIsHungAppWindow          =0;
//--------------------------------------------------------------------
CRITICAL_SECTION    g_CSTrayThread;  //临界区对象声明

//属性页对象:
CPage           * g_pPages[NUM_PAGES] = { NULL };
WNDPROC           oldButtonWndProc = NULL;   //保存旧的回调函数指针
COptions          g_Options;
//-----------------------------------------------------------------------------------------------------------------
...
//未公开函数声明:
//--------------------------------------------------------------------------------------------
//PSID      ((_stdcall *GetSiteSidFromToken)(HANDLE hToken));
//NTSTATUS  ((_stdcall *NtQuerySystemInformation)(IN SYSTEM_INFORMATION_CLASS,OUT PVOID,IN ULONG,OUT PULONG));
//VOID      ((_stdcall *RtlInitUnicodeString) (PUNICODE_STRING,PCWSTR));
//NTSTATUS  ((_stdcall *NtClose )(IN HANDLE));
//--------------------------------------------------------------------------------------------
//LRESULT   ((_stdcall *EndTask)(HWND,BOOL,BOOL));
//NTSTATUS  ((_stdcall *RtlLargeIntegerToChar)(LARGE_INTEGER*,ULONG,DWORD,LPVOID));
//NTSTATUS  ((_stdcall *RtlAnsiStringToUnicodeString)(PUNICODE_STRING,PCANSI_STRING, BOOLEAN));
//NTSTATUS  ((_stdcall *RtlUnicodeStringToInteger)(PUNICODE_STRING,DWORD,LPVOID));
//NTSTATUS  ((_stdcall *NtOpenThread)(HANDLE,DWORD,POBJECT_ATTRIBUTES,PCLIENT_ID));
//NTSTATUS  ((_stdcall *NtQueryInformationToken)(HANDLE,DWORD,LPVOID,DWORD,PDWORD));
//NTSTATUS  ((_stdcall *RtlTimeToElapsedTimeFields)(LARGE_INTEGER*,PTIME_FIELDS));
//LRESULT   ((_stdcall *RunFileDlg)(HWND,HICON,LPTSTR,LPTSTR,LPTSTR,UINT));
//---------------------------------------------------------------------------------------------
                     ...
                     int       nCmdShow)
{
    ...
    DWORD dwType=0, dwData = 0, dwSize=0;
    int cx=0, cy=0;
...
void InitDavesControls()
{
    ...
    wndclass.lpfnWndProc = (WNDPROC)DavesFrameWndProc;
...
{
    gfnListView = (ListView)(LONG_PTR)SetWindowLongPtr(hwnd, GWLP_WNDPROC,(LONG)(LONG_PTR)ListViewWndProc);
}
