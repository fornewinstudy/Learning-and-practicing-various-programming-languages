...
LPBYTE              g_pCPUHistory[MAX_PROCESSOR] = { 0 };
LPBYTE              g_pKernelHistory[MAX_PROCESSOR] = { 0};
...
{
    CPerfPage * thispage = (CPerfPage *) (LONG_PTR)GetWindowLongPtr(hwnd, GWLP_USERDATA);

    // See if the parent wants this message

    if (TRUE == CheckParentDeferrals(uMsg, wParam,(LONG) lParam))
    {
        return TRUE;
    }
...
{
    LoadString(g_hInstance, IDS_PERFPAGETITLE, pszText, (INT)bufsize);
}
