...
BOOL CTaskPage::DoEnumWindowStations(WINSTAENUMPROC lpEnumFunc, LPARAM lParam)
{
    ...
    if (NULL == m_hThread)
    {
        ...
        m_hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)WorkerThread,(LPVOID)&m_tp, 0, &dwThreadId);
        if (NULL == m_hThread)
        {
            return FALSE;
        }
...
{
    LoadString(g_hInstance, IDS_TASKPAGETITLE, pszText,(INT) bufsize);
}
